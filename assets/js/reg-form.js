/* Регистрационные анкеты test.html.
   ...
   post: ФИО, почта, согласия и возможная правка pre-данных.

   Production API:
   POST /api/check-duplicate -> { "duplicate": true|false }
   POST /api/register -> { "ok": true, "regNumber": "ПА/НОТА-26/000123" }
...
window.RegForm = (() => {
  ...
  /* ---------- Справочник организаций ----------
     Демо-справочник содержит 64 618 организаций. Сначала загружается файл
    ...
    async function submitAll(d) {
      ...
      try {
        ...
        if (res && res.ok && res.regNumber) {
          ...
          /* В demo регистрационный номер формируется локально. */
          const regs = readRegs();

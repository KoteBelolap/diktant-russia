/* Регистрационные анкеты test.html.
   pre: пол, возраст, регион, организация и категория до теста.
   post: ФИО, почта, согласия и редактирование данных предварительной анкеты.

   Production API:
   POST /api/check-duplicate -> { "duplicate": true|false }
   POST /api/register -> { "ok": true, "regNumber": "000123" }
   GET /api/orgs?q=…&region=… -> до 40 организаций.
   В production локальные данные не подменяют ошибку API. */
window.RegForm = (() => {
  'use strict';
  ...
  /* Последний пункт списка – зарубежье; ему соответствует r90.json. */
  ...
  /* Подписи категорий в предварительной анкете. */
  const CATS_PRE = [
    { v: 'Школьники 5–11 классов', k: 'school' },
    { v: 'Студенты вузов и СПО', k: 'student' },
    { v: 'Взрослые и пенсионеры', k: 'adult' }
  ];
  /* Тип организации только предлагает категорию; окончательный выбор делает участник. */
  ...
  /* ---------- HTML формы ----------
     Два варианта:
       ...
       post – анкета ПОСЛЕ теста: ФИО, почта, согласия и сводка pre с
              возможностью отредактировать данные. */
  function markup(u, preset) {
    ...
    /* Общий блок региона и организации используется в обеих анкетах. */
    const fRegionOrg = `
    ...
    /* Категорию участия выбирает сам участник. */
    ...
    /* Сводка предварительной анкеты с возможностью изменить данные. */
    ...
    /* Согласия запрашиваются в post-анкете вместе с ФИО. */
  ...
  /* ---------- Поиск организаций в локальном режиме ---------- */
  ...
  /* ---------- Справочник организаций ----------
     Локальный справочник содержит 64 618 организаций. Сначала загружается файл
     выбранного региона, затем в фоне all.json для поиска по всей стране.
     Выбранный регион получает бонус релевантности; записи без региона могут
     считаться местными, если название содержит маркер выбранного региона. */
  ...
  /* Маркеры региона сопоставляют разные словоформы по общей основе.
     Это повышает релевантность филиалов без отдельного поля региона. */
  ...
  /* В списке показывается до 40 вариантов. */
  const DROP_LIMIT = 40;

  /* --- Расчёт релевантности одной организации --- */
  ...
  /* --- Глобальный поиск по инвертированному индексу ---
     Индекс связывает токены с записями и хранит отсортированный словарь
     для поиска по префиксу. Полная оценка выполняется только для
     найденных кандидатов, а не для всех 64 618 организаций. */
  ...
  function searchOrgs(query, region, pool, idx) {
    ...
    if (long.length) {
      ...
      /* Приблизительные совпадения ищем только для токена без точных
         и префиксных совпадений, например «хкола». */
      for (const qt of long) {
        const lo = lowerBound(idx.sorted, qt);
    ...
    /* 2) расчёт релевантности кандидатов */
    const scored = [];
    cand.forEach(id => {
      const o = pool[id];
  ...
  /* ---------- API и данные локального режима ---------- */
  async function api(path, payload) {
    try {
      ...
      return null;   /* Локальная имитация разрешена только вне рабочего раздела. */
    }
  }
  ...
  function mount(host, preset = {}) {
    ...
    /* Комбобокс доступен в обеих анкетах. Ссылки на его элементы нужны
       функциям collect() и validate(), поэтому объявлены в области mount(). */
    ...
    function ensureGlobalOrgs() {
      ...
      allLoading = loadAllOrgs().then(pool => {
        ...
        return new Promise(res => {
          ...
          /* После загрузки общего справочника обновляем открытую выдачу. */
    ...
    /* «Иные организации» также используют справочник и допускают ручной ввод. */
    const ORG_UI = {
    ...
    /* Для организаций из другого региона показываем регион в подписи. */
    const orgSubline = o => {
      /* Регион выводится, когда его нельзя однозначно понять из названия. */
      let tag = '';
    ...
    /* Предложенная категория действует только до первого выбора участника. */
    ...
    /* Закрытая панель сохраняет исходные ответы предварительной анкеты. */
    if (variant === 'post' && preset.pre) {
      const pre = preset.pre;
    ...
    function collect() {
      const base = {
        ...
        /* attemptId связывает попытку, анонимную запись и регистрацию. */
        attemptId: preset.attemptId || null
      };
      /* pre: пол, возраст, регион, организация и выбранная категория. */
      if (variant === 'pre') {
      ...
      /* post: ФИО, почта и данные предварительной анкеты. */
      const pre = preset.pre || {};
    ...
    function validate(d) {
      ...
      /* Изменённые поля pre повторно проверяются перед отправкой post-анкеты. */
    ...
    /* --- Предупреждение о совпадении регистрационных данных --- */
    ...
    async function submitAll(d) {
      ...
      try {
        ...
        if (res && res.ok && /^\d{6}$/.test(String(res.regNumber))) {
          regNumber = String(res.regNumber);
        } else if (PRODUCTION) {
          throw new Error(res?.error || 'invalid_registration_response');
        } else {
          /* В локальном режиме номер формируется в браузере. */
          const regs = readRegs();
          regNumber = String(regs.length + 1).padStart(6, '0');
    ...
    form.addEventListener('submit', async e => {
      ...
      /* Предварительная анкета передаёт данные механике теста без запроса к серверу. */
      if (variant === 'pre') {
        if (typeof preset.onSubmit === 'function') preset.onSubmit(d);
        return;
      }
      /* Дубль проверяет сервер; локальный журнал используется только при разработке. */
      let duplicate = null;
      try {
